<?php
		...
		$perm = testarPermissao('INCLUIR CADASTRO DE NCM');
								...
								<table class="table table-hover table-striped tabela-registro" id="tabela">
									<thead>
										<tr>
											<th>NCM</th>
											<th>Descrição</th>
											<th class="hidden-xs">Monofasico</th>
										</tr>
									</thead>
									<tbody>
									<?php
										...
										if (empty($pesquisa)) {
											$sql = "select * from ncm order by ncm limit " . $limite . " offset " . (($pagina-1)*$limite);
										} else {
											$sql = "select * from ncm where ncm like '" . $pesquisa . "%' order by ncm limit " . $limite . " offset " . (($pagina-1)*$limite);
										}
										...
										if ($rows != null) {
											foreach ($rows as $row) {
												...
												echo "<td class=\"hidden-xs\">" . $row['monofasico'] . "</td>";
												echo "</tr>";
											}
										}	
										...
										// Paginaçao
										if (empty($pesquisa)) {
											$sql = "select count(*) as num from ncm";
										} else {
											$sql = "select count(*) as num from ncm where ncm like '" . $pesquisa . "%';";
										}
