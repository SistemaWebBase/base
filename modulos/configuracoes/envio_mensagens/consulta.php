							...
							<div class="panel-body">
								...
								<table class="table table-hover table-striped tabela-registro" id="tabela">
									...
									<?php
										...
										if (empty($pesquisa)) {
											$sql = "select * from envio_mensagens order by usuario limit " . $limite . " offset " . (($pagina-1)*$limite);
										} else {
											$sql = "select * from envio_mensagens where usuario like '" . $pesquisa . "%' order by usuario limit " . $limite . " offset " . (($pagina-1)*$limite);
										}
										...
										if ($rows != null) {
											foreach ($rows as $row) {
												echo "<tr onclick=\"abrirCadastro('" . $row['usuario'] . "', '" . $row['nome_mensagem'] . "', '" . assinarParametros("usuario=" . $row['usuario'] . "&nome_mensagem=" . $row['nome_mensagem']) . "');\">";
												echo "<td>" . $row['usuario'] . "</td>";
												echo "<td>" . $row['nome_mensagem'] . "</td>";
												echo "</tr>";
											}
										}	
										...
										// Paginaçao
										if (empty($pesquisa)) {
											$sql = "select count(*) as num from envio_mensagens";
										} else {
											$sql = "select count(*) as num from envio_mensagens where usuario like '" . $pesquisa . "%';";
										}
